...
export interface IInstructorRepository extends IBaseRepository<IInstructor> {
    ...
    updateInstructorStatus(id:string, updates:object):Promise<IInstructor| null>
    ...
    findInstructorsByUserId(instructorIds: string[], skip:number, limit: number, searchQuery?:string): Promise<IInstructor[]>
    findAllInstructors(skip: number, limit: number, searchQuery?: string): Promise<IInstructor[] >
}